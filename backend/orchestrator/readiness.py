...
async def resolve_node_readiness(
    ...
    # Special condition for Backend Engineer rework: requires at least failure_context, review_feedback, test_failure, or security_finding for attempt=node_attempt
    if node.agent_role == "backend_engineer" and node_attempt > 1:
        has_rework_feedback = any(
            k in resolved_inputs for k in ("failure_context", "review_feedback", "test_failure", "security_finding")
        )
    ...
    # Special condition for DevOps Engineer rework: requires at least devops_finding or review_feedback for attempt=node_attempt
    if node.agent_role == "devops_engineer" and node_attempt > 1:
        has_rework_feedback = any(
            k in resolved_inputs for k in ("devops_finding", "review_feedback")
        )
        if not has_rework_feedback:
            return False, {}
